...
def chat_session(request, chat_id=None): # 
    ...
    try:
        refresh_vector_store(force_refresh=False)
    except Exception:
        logger.exception("Kunde inte initiera vector store")
    ...
    if request.method == "POST":
        ...
        # calls ai-modell
        try:
            ai_message = get_ai_response_modular(user_message, conversation=conversation, chat_history=chat_history)
        except Exception:
            import logging
            logging.getLogger("rag").exception("AI-svar misslyckades")
